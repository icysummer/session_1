...
######### Task11 ############
mpg6 %>% rename(max_vpc_1999 = `1999`, max_vpc_2008 = `2008`) %>% gather(variable, value, max_vpc_1999, max_vpc_2008, change) %>% as.data.frame
...
#Joining, by = "carrier"
avg_goog_weather_delays <- good_weather_delays %>% group_by(carrier) %>% summarise(a_dep_delay = mean(dep_delay)) %>% arrange(a_dep_delay) %>% inner_join(airlines)
